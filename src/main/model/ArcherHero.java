...
public class ArcherHero extends Hero {
    ...
    public int firstSkill() {
        ...
        int damage = (int) Math.round(basicAttack() * (1 + (1.25 * getAgility())));
    ...
    public int secondSkill() {
        ...
        int damage = (int) Math.round(basicAttack() * (1 + (1.5 * getAgility())));
        super.usedSkill(2);
        return damage;
    }
    ...
    public int thirdSkill() {
        ...
        int damage;
        if (Math.random() >= 0.5) {
            damage = (int) Math.round(basicAttack() * 3 * (1 + (1.5 * getAgility())));
        } else {
            damage = 0;
        }
